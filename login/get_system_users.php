<?php
	...
	$page = isset($_POST['page']) ? intval($_POST['page']) : 1; #是否设置了页码,如有按照设置页码,没有的话当前页码设置为1
	$rows = isset($_POST['rows']) ? intval($_POST['rows']) : 10; #是否设置了页行数,如有的话按照设置的行数,否则每页的行数设为10
	$offset = ($page-1)*$rows;	#根据当前页码获取偏移量
	...
	while($row = mysql_fetch_object($rs)){
		array_push($items, $row);	#向items数组的尾部逐一循环添加元素（入栈），然后返回新数组的长度 
	}
	$result["rows"] = $items;		#带有字符串键的数组,存放实际的记录资料的阵列集
